...
```{r}
ggplot(credit_data_recode, aes(x = job, y = credit_amount, color = job)) +
  geom_boxplot() +
...
```{r}
cor_age_credit <- cor(credit_data_recode$age, credit_data_recode$credit_amount)

ggplot(credit_data_recode, aes(x = age, y = credit_amount, color = assessment)) +
  geom_point() +
  geom_smooth(method = 'lm', se = F) +
  labs(x = "Age of Applicants", 
       y = "Credit Score of Applicants", 
       title = paste0("Relationship between Credit Score & Age; Correlation: ", round(cor_age_credit, 2))
       ) + theme_minimal() +
  scale_color_manual(values = c('red', 'green'))
```

```{r}
cor_inst_credit <- cor(credit_data_recode$installment_rate, credit_data_recode$credit_amount)

ggplot(credit_data_recode, aes(x = installment_rate, y = credit_amount, color = assessment)) +
  geom_point() +
  # geom_smooth(method = 'lm', se = F) +
  labs(x = "Installment of Applicants", 
       y = "Credit Score of Applicants", 
       title = paste0("Relationship between Credit Score & installment."), 
       subtitle = paste0("Correlation: ", round(cor_inst_credit, 2))
       ) + theme_minimal() +
  scale_color_manual(values = c('red', 'green'))
```
